 ...
 * */
@Component
public class EmailPasswordLoginOperation implements LoginOperation{
    ...
    public ResultModel<String> login(UserLoginDto userLoginDto,String ip) {
        Optional<UserEntity> userEntityOptional = Optional.ofNullable(userMapper.selectOne(
                new QueryWrapper<UserEntity>()
                        ...
                        .or(qw -> {
                            qw.eq("user_email", userLoginDto.getUserId()).eq("user_pwd", userLoginDto.getUserPwd());
                        })
        ));
        if(!userEntityOptional.isPresent()){
            return ResultModel.buildFailResultModel("密码输入错误，请重新输入！");
        }
